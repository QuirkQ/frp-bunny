...
RUN apk upgrade --no-cache \
    ...
    && addgroup -S -g 1000 frp \
    && adduser -S -G frp -H -s /sbin/nologin -u 1000 frp \
    && mkdir -p /etc/frp \
    && chmod 777 /etc/frp
...
LABEL org.opencontainers.image.title="frp-bunny" \
    org.opencontainers.image.description="Hardened frp server and client for Bunny Magic Containers tunneling" \
